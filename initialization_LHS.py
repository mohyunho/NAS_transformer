...
from utils.model_sample_creator import geno2sample, geno2snip
...
def main():
    ...
    parser.add_argument('--pop', type=int, default=10, required=False, help='population size of EA')
    parser.add_argument('--gen', type=int, default=0, required=False, help='generations of evolution')
    ...
    pic_ind_dir = os.path.join(pic_dir, 'pic_ind_initial_%s_%s_%s_%s' %(numb_valX, pt, n_samples, subdata))
    ...
    l_bounds = [6, 6, 6, 6, 6, 6, 1, 1, 1, 1, 1]
    u_bounds = [25, 25 ,25 ,25 ,25 ,25, 16 ,16 ,16 ,3, 3]

    sample_array_int = sampler.integers(l_bounds=l_bounds, u_bounds=u_bounds, n=n_samples, endpoint=True, workers=1)

    # sample_array = qmc.scale(sample, l_bounds, u_bounds)
    # sample_array_int = sample_array.astype(int) # n_samples X n_parameters

    print ("sample_array_int", sample_array_int)
    print ("sample_array_int.shape", sample_array_int.shape)
    ...
    log_df['idx'] = np.arange(n_samples)

    for idx in range(n_parameters):
        p_idx = idx+1
        log_df['params_%s' %p_idx] = sample_array_int[:,idx]

    log_df['fitness_1'] = np.zeros(n_samples)
    log_df['gen'] = np.zeros(n_samples)
    ...
    for i, s in enumerate(sample_array_int):
        ...
        early_stopping = EarlyStopping(patience=pt, delta= 0.1, verbose=False)
        ...
        geno_snip = copy.deepcopy(genotype)
        geno_snip = geno_snip.tolist()

        snip_value= geno2snip(geno_snip, X_train, Y_train,  epochs, bs, window_Size)
        print ("snip: ", snip_value)
        ...
        min_test_score_value = test_score_list[min_val_idx]

        print ("min_val_value", min_val_value)
        print ("min_test_value", min_test_value)
        print ("min_train_value", min_train_value)
        print ("min_test_score_value", min_test_score_value)

        ind_val_lst.append(min_val_value)  
        ind_test_lst.append(min_test_value)
        ind_train_lst.append(min_train_value)

        min_avg_value = (min_val_value+min_train_value)/2.0
        ind_avg_lst.append( min_avg_value )

        ind_test_score_lst.append(min_test_score_value)

        ind_snip_lst.append(snip_value)
        ...
        plt.text(min_val_idx+1.0, min_train_value+3, "%s" %min_train_value)
        plt.text(min_val_idx+1.0, min_train_value+1.5, "%s" %min_avg_value,  color='blue')
    ...
    log_df["test_rmse"] = ind_test_lst
    log_df["val_rmse"] = ind_val_lst
    log_df["train_rmse"] = ind_train_lst
    log_df["avg"]  = ind_avg_lst
    log_df["num_params"] = num_params
    log_df["train_time"] = ind_train_time

    log_df["snip_values"] = ind_snip_lst
    log_df["test_scores"] = ind_test_score_lst
    log_df["found_lr"] = lr_lst
    


    # mutate_log_path = os.path.join(log_dir_path, 'mute_log_test_%s_%s_%s_%s_%s.csv' % (pop_size, n_generations, obj, subdata, trial))

    test_filename = os.path.join(log_dir_path, 'initialization_rmse_%s_%s_%s_%s_%s_%s_%s.csv' % (validation_numb, pt, n_samples,  obj, subdata, ep, trial))
    log_df.to_csv(test_filename, index=False)
