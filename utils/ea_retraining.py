...
def ind_gd (population, history_retrain, topk=10):
...
def eaSimple(population, toolbox, cxpb, mutpb, ngen, sel_op, min_query, stats=None,
    ...
    print ("fitness update of current population after the update")
    ...
    if sel_op == "nsga2":
        ...
        for gen in range(1, ngen + 1):
            ...
            print ("map the fitness to the individuals in offspring with the predictor")
    ...
    df_retrain.to_csv("retrain_log_FD00.csv", index=False)
...
class GeneticAlgorithm:
    ...
    def run(self):
        ...
        # self.toolbox.register("update_lhc_sample", self.task.update_pred_sample)
