...
class AttentionBlock(torch.nn.Module):
    def __init__(self, dim_m, dim_val, dim_attn):
        super(AttentionBlock, self).__init__()
        self.value = Value(dim_m, dim_val)  
        self.key = Key(dim_m, dim_attn)  
        self.query = Query(dim_m, dim_attn) 
...
class MultiHeadAttentionBlock(torch.nn.Module):  
    def __init__(self, dim_m, dim_val, dim_attn, n_heads): 
        super(MultiHeadAttentionBlock, self).__init__()
        self.heads = []
        for i in range(n_heads):
            self.heads.append(AttentionBlock(dim_m, dim_val,  dim_attn))       
        self.heads = nn.ModuleList(self.heads)
        self.fc = nn.Linear(n_heads * dim_val, dim_m, bias = False, device="cuda")
...
class Sensor_MultiHeadAttentionBlock(torch.nn.Module): 
    def __init__(self, dim_m, dim_val, dim_attn, n_heads): 
        ...
        self.heads = []
        for i in range(n_heads):
            self.heads.append(Sensor_AttentionBlock(dim_m, dim_val,  dim_attn))       
        self.heads = nn.ModuleList(self.heads)      
        self.fc = nn.Linear(n_heads * dim_val, dim_m, bias = False, device="cuda")
...
class TimeStepMultiHeadAttentionBlock(torch.nn.Module): 
    def __init__(self, dim_m, dim_val, dim_attn, n_heads): 
        ...
        self.heads = []
        for i in range(n_heads):
            self.heads.append(time_step_AttentionBlock(dim_m, dim_val,  dim_attn))        
        self.heads = nn.ModuleList(self.heads)        
        self.fc = nn.Linear(n_heads * dim_val, dim_m, bias = False, device="cuda")
...
class Value(torch.nn.Module):
    def __init__(self, dim_input, dim_val):
        ...
        self.fc1 = nn.Linear(dim_input, dim_val, bias = False, device="cuda")
...
class Key(torch.nn.Module):
    def __init__(self, dim_input, dim_attn):
        ...
        self.fc1 = nn.Linear(dim_input, dim_attn, bias = False, device="cuda")
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, drop_rate=0.2, max_len=512):
